...
syn_%: check_env
	...
	@mkdir -p $(MAP)
	@$(SYNTH_TOOL) -d -p 'read_verilog -sv -I $(SRC) $(SRC)/$*.sv; synth -top $*; dfflibmap -liberty $(LIBERTY); abc -liberty $(LIBERTY); clean; write_verilog -noattr -noexpr -nohex -nodec -defparam $(MAP)/$*.v' > $*.log
	@echo -e "\nSynthesis complete!\n"
