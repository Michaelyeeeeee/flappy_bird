...
module flappy_game (
        ...
        // Clock Divider to slow down the game hz
        logic [18:0] tick_cnt;
        logic game_tick; 
        ...
        // Random Number Generator to deterine pipe heights
        logic [7:0] rng = 8'hAB;
        ...
        // Failsafe to make sure the button works
        logic jump_pending;
        always_ff @(posedge clk) begin
        ...
        always_ff @(posedge clk) if( game_tick ) begin
                if( state == STATE_READY ) begin
                        ...
                        pipe_x[0] <= 500; pipe_x[1] <= 800; pipe_x[2] <= 1100;
                        pipe_gap_y[0] <= 150; pipe_gap_y[1] <= 250; pipe_gap_y[2] <= 100;
                end else if( state == STATE_PLAY ) begin
        ...
        always_ff @(posedge clk) begin
                ...
                if( x_val >= `BIRD_X && x_val < `BIRD_X + `BIRD_W 
                 && y_val >= bird_y  && y_val < bird_y + `BIRD_H ) 
                        bird_body <= 1;

                //bird eye
                bird_eye <= 0;
                if( x_val >= `BIRD_X + 18 && x_val < `BIRD_X + 20 
                 && y_val >= bird_y + 5   && y_val < bird_y + 8 )
                        bird_eye <= 1;

                //bing wing
                bird_wing <= 0;
                if( x_val >= `BIRD_X + 2 && x_val < `BIRD_X + 14 
                 && y_val >= bird_y + 12 && y_val < bird_y + 20 )
                        bird_wing <= 1;
                ...
                pipe_pixel <= 0;
                for( int i = 0; i < 3; i++ ) begin
                        if( ($signed({2'b0, x_val}) >= pipe_x[i]) && ($signed({2'b0, x_val}) < pipe_x[i] + `PIPE_W) )
                                if( y_val < pipe_gap_y[i] || y_val > pipe_gap_y[i] + `GAP_SIZE )
                                        pipe_pixel <= 1;
                end

                //priority mux for colors
                     if( progress_bar ) rgb_out <= 3'b111; //White progress bar
                else if( bird_eye )     rgb_out <= 3'b000; //Black Eye
                else if( bird_wing )    rgb_out <= 3'b111; //White Wing
                else if( bird_body )    rgb_out <= (state == STATE_DEAD) ? 3'b100 : 3'b110; //Yellow if alive, Red if dead
                else if( pipe_pixel )   rgb_out <= 3'b010; //Green Pipe
